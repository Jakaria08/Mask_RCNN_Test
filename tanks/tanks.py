"""
Mask R-CNN
Train on the tank segmentation
...
class TanksConfig(Config):
    """Configuration for training on the tanks segmentation dataset."""
    ...
    STEPS_PER_EPOCH = (429 - 24) // IMAGES_PER_GPU
...
class TanksDataset(utils.Dataset):

    def load_Tanks(self, dataset_dir, subset):
        """Load a subset of the Tanks dataset.

        dataset_dir: Root directory of the dataset
        subset: Subset to load. Either the name of the sub-directory,
                such as:
                * train: train excluding validation images
        ...
        if subset == "test":
            for img_file in glob.glob(dataset_dir + '/*.png'):
        ...
        for image_id in image_ids:
            coordinates = image_id.split('_')
            length = len(coordinates)
            x_co = coordinates[length - 8]
            y_co = coordinates[length - 9].split('-')[1]
            image_id_new = int(y_co + x_co)
            #print(image_id_new)
            self.add_image(
                "tanks",
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        main_dir_name = os.path.splitext(base)[0]
        upper_dir_name = os.path.dirname(info['path'])
        mask_dir = os.path.join(upper_dir_name,main_dir_name)
